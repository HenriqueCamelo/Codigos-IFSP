...
public class ServeletDespesa extends HttpServlet {
	...
	private void InfomaçõesDespesa(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		DBQuery query = new DBQuery("despesas","idDespesa,dataCompra, idFornecedor, endereco, cidade, bairro, CNPJCPF, valor, numeroParcelas, fazenda","idDespesa");
		ResultSet rs = query.select("idDespesa > 0 AND fazenda =" + "'"+fazenda+"'");
		...
		try {
			...
			despesa.setFazenda(rs.getString("fazenda"));
        ...
        response.getWriter().write(json);
	        ...
	        e.printStackTrace();
	        response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Erro ao acessar informações das despesas");
	    }
		}
	...
	private void CadastrarDespesa(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		String fazendaDespesa = request.getParameter("fazendaDespesa");
		
		DBQuery query = new DBQuery("despesas","dataCompra,idFornecedor,endereco,cidade,bairro,CNPJCPF,valor,numeroParcelas,fazenda","idDespesa");
		...
		response.sendRedirect("./View/Despesas.jsp?status=sucesso");
		}catch (Exception e) {
	        e.printStackTrace();
	        response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Erro ao cadastrar despesa");
	    }
	...
	private void DeletarDespesa(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		response.sendRedirect("./View/informacoesDespesa.jsp?status=sucesso");
		}catch (Exception e) {
	        e.printStackTrace();
	        response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Erro ao atualizar animal");
	    }
	}
	...
	private void AtualizarDespesa(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		response.sendRedirect("./View/informacoesDespesa.jsp?status=sucesso");
	        ...
	        e.printStackTrace();
	        response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Erro ao atualizar despesa");
	    }
	}
